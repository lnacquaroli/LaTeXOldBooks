...
alpha = 30.3
x0 = A0*np.cos(alpha)
t = np.arange(0, 0.935, 0.001)
A = A0*np.exp(-beta*t + x0)
...
ax.plot(t, x, c=c[0], lw=lw, ls="solid")
ax.plot(t, A, c=c[2], lw=lw, ls="dashed")
ax.plot(t, -A, c=c[2], lw=lw, ls="dashed")
ax.set_xticklabels([])
ax.set_xticks([])
ax.set_yticklabels([])
...
#
plt.savefig(path + r"\fig_7_19_.pdf", bbox_inches="tight")