...
v = -A*np.sin(np.rad2deg(omega_0*t + alpha))
a = -A*np.cos(np.rad2deg(omega_0*t + alpha))
...
ax[1].plot(t, v, c=c[1], lw=lw, ls="solid")
...
ax[1].set_ylabel(R"$v$", fontsize=fs, rotation=0, color=c[1])#"Fluence, $\si{\milli\joule\per\square\centi\meter}$", fontsize=fs)
ax[1].yaxis.set_label_coords(0.05,1.1)
ax[1].text(-0.1, 0.92, r"$+A$", fontsize=fs)
ax[1].text(-0.1, -0.92, r"$-A$", fontsize=fs)

ax[2].spines["bottom"].set_position("zero")
ax[2].plot(t, a, c=c[3], lw=lw, ls="solid")
...
ax[2].set_ylabel(R"$a$", fontsize=fs, rotation=0, color=c[3])#"Fluence, $\si{\milli\joule\per\square\centi\meter}$", fontsize=fs)
...
#
plt.savefig(path + r"\fig_7_5.pdf", bbox_inches="tight")