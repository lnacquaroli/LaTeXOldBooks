...
# Data
A = 1
t = np.arange(0, 1.37, 0.0001)
omega = 0.1
delta_omega = omega/10.0
m1 = 2.0*A*np.cos(np.rad2deg(delta_omega*t/2))
m2 = np.cos(np.rad2deg(omega*t))
x = m1*m2
omega = 2.0
delta_omega = omega/10.0
m1 = 2.0*A*np.cos(np.rad2deg(delta_omega*t/2))*0.93
m2 = np.cos(np.rad2deg(omega*t))
x2 = m1*m2
...
ax[0].plot(t, x2, c=c[1], lw=lw, ls="solid")
ax[0].plot(t, x, c=c[0], lw=lw, ls="solid")
ax[0].plot(t, -x, c=c[0], lw=lw, ls="dashed")
...
ax[1].plot(t, np.abs(x), c=c[3], lw=lw, ls="solid")
...
#
plt.savefig(path + r"\fig_7_11_.pdf", bbox_inches="tight")